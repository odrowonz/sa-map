    ...
    <header class="sticky top-0 z-50 shrink-0 border-b border-slate-200 bg-white shadow-sm">
        <div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-6 py-4">
            ...
            <nav class="flex flex-wrap items-center gap-3 text-sm">
                @auth
                    <a href="{{ route('dashboard') }}" class="font-medium text-slate-800 hover:text-slate-600">Личный кабинет</a>
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="inline">
                        @csrf
                        <button type="submit" class="text-slate-700 underline hover:text-slate-900">Выйти</button>
                    </form>
                @else
                    <a href="{{ route('login') }}" class="text-slate-700 hover:text-slate-900">Вход</a>
                    <a href="{{ route('register') }}" class="text-slate-700 hover:text-slate-900">Регистрация</a>
                @endauth
            </nav>
        </div>
    </header>
