...
@section('title', __('sa.njk.create_title').' — '.config('app.name'))
...
@section('workspace_toolbar')
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="min-w-0">
            <h1 class="truncate text-lg font-bold uppercase tracking-wide text-slate-800">{{ __('sa.njk.create_title') }}</h1>
...
@section('workspace_main')
    <div class="px-4 py-6 sm:px-8 lg:px-10">
        <form id="njk-template-form" method="post" action="{{ route('njk-templates.store') }}" class="max-w-4xl">
            @csrf
