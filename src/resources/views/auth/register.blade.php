...
@section('workspace_toolbar')
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="min-w-0">
            <h1 class="truncate text-lg font-bold uppercase tracking-wide text-slate-800">{{ __('sa.auth.register_title') }}</h1>
    ...
    <div class="px-4 py-6 sm:px-8 lg:px-10">
        <div class="mx-auto max-w-lg">
            <div class="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm sm:p-8">
                <span class="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-slate-600">{{ __('sa.auth.register_title') }}</span>
                <h2 class="mt-4 text-xl font-bold text-slate-900">{{ __('sa.auth.register_title') }}</h2>
                <form method="POST" action="{{ route('register') }}" class="mt-6 space-y-5">
                    @csrf
                    <div>
                        <label for="name" class="block text-xs font-bold uppercase tracking-wide text-slate-500">{{ __('sa.auth.name_optional') }}</label>
                        <input
                            id="name"
                            name="name"
                            type="text"
                    ...
                    <div>
                        <label for="password_confirmation" class="block text-xs font-bold uppercase tracking-wide text-slate-500">{{ __('sa.auth.password_confirm') }}</label>
                        <input
                ...
                <p class="mt-8 border-t border-slate-100 pt-6 text-sm text-slate-600">
                    {{ __('sa.auth.has_account') }}
                    <a href="{{ route('login') }}" class="font-semibold text-slate-800 underline decoration-slate-300 underline-offset-2 hover:text-slate-950">{{ __('sa.auth.login_title') }}</a>
                </p>
            </div>
        </div>
    </div>
